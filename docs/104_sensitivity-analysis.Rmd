...
```{r}
mrks<-read.delim("sensitivity/parent-courter_supergene_crs2_ncrs8_1_markers.txt")
qtls<-read.delim("sensitivity/parent-courter_supergene_crs2_ncrs8_1_qtlinfo.txt")
...
plot(c(0,1),c(0,1),bty='n',type='n',xlab="",ylab="Allele Frequency")
points(jitter(rep(0.25,length(grep("Courter",colnames(qtls))))),qtl_af[nrow(qtl_af),paste("Marker",qtls[1,grep("Courter",colnames(qtls))],sep="")],col=cols["courter"])
points(jitter(rep(0.75,length(grep("Parent",colnames(qtls))))),qtl_af[nrow(qtl_af),paste("Marker",qtls[1,grep("Parent",colnames(qtls))],sep="")],col=cols["parent"])
