...
plot(log(as.numeric(cpu_all_freqs$Courter2NonRS)),
     cpu_all_freqs$Parent2NonSurvival,xlim=c(-2,2),ylim=c(1,10),axes = FALSE,
     xlab="Relative reproductive allocation (courter/non-courter)",
     ylab="Relative nest survival (parent/non-parent)",type='n')
abline(h=c(1.5,2.1,2.75,3.75,4.8,5.5,6.5,7.5,8.5,9.5),col="grey",xpd=FALSE)
axis(2,at=as.numeric(levels(as.factor(cpu_all_freqs$Parent2NonSurvival))),lty=0,las=1)
abline(v=seq(-1.75,2,0.7),col="grey",xpd=FALSE)
axis(1,at=as.numeric(levels(as.factor(log(as.numeric(cpu_all_freqs$Courter2NonRS))))),
     labels = levels(as.factor(cpu_all_freqs$Courter2NonRS)),lty=0)
points(jitter(log(as.numeric(cpu_all_freqs$Courter2NonRS))),cpu_all_freqs$Parent2NonSurvival+0.2,
       col=alpha(cols2["CP"],cpu_all_freqs$FreqCP),pch=5)
points(jitter(log(as.numeric(cpu_all_freqs$Courter2NonRS))),cpu_all_freqs$Parent2NonSurvival+0.075,
       col=alpha(cols2["CNP"],cpu_all_freqs$FreqCNp),pch=2)
points(jitter(log(as.numeric(cpu_all_freqs$Courter2NonRS))),cpu_all_freqs$Parent2NonSurvival-0.075,
       col=alpha(cols2["NCP"],cpu_all_freqs$FreqNcP),pch=1)
points(jitter(log(as.numeric(cpu_all_freqs$Courter2NonRS))),cpu_all_freqs$Parent2NonSurvival-0.2,
       col=alpha(cols2["NCNP"],cpu_all_freqs$FreqNcNp),pch=0)
par(fig=c(0, 1, 0, 1), oma=c(0, 0, 0, 0), mar=c(0, 0, 0, 0), new=TRUE)
plot(0, 0, type='n', bty='n', xaxt='n', yaxt='n')
legend("top",bty='n',legend = c("Courter/Parent","Courter/Non-parent","Non-courter/Parent","Non-courter/Non-parent"),
       col=cols2[c("CP","CNP","NCP","NCNP")],pch=c(5,2,1,0),xpd = TRUE,ncol=2)
dev.off()
```
