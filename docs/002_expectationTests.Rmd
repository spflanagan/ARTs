...
Of the `r length(cu)` replicates, all of them reached an equilibrium by 10000 generations. 
...
```{r courter_table}
kable(courter.final.freqs,caption = "Frequency of courters in final generation",
      row.names = TRUE)
```
...
#get the files
trait.files<-list.files(pattern="^courter_linked_\\d+_traits.txt")
...
ps<-plot.parent.reps(pattern="^parent_supergene_\\d+_summary.txt",cols)
```
...
```{r courtParent_morphfreqs_supergenes,echo=FALSE,out.extra='',fig.pos="H",fig.cap="Frequency of the 4 morphs in each rep with supergenes",fig.height=10,fig.width=6}
