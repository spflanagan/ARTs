...
mermaid("
  graph LR
    ...
    B[Males Fertilize Eggs]-->D[Nest Survival]
    D[Nest Survival]-->E[Viability Selection on Progeny]
    E[Viability Selection on Progeny]-->F[Stochastic Survival to Adulthood]
    F-->A
  ")
```
...
Females choose nests based on whether the male is a courter or not, and they all prefer courters all of the time. The only way that non-courters produce offspring is through sneaking. Because parental care is not incorporated in this model, all nests survive.

```{r unlinked_courter}
courter.files<-list.files(pattern="^courter-nogenetics_\\d+_summary.txt")
#set up colors
mycols <- colorRampPalette(c("grey",cols["courter"],"black"))(12)[2:11]
...
text(x=10000,y=1.125,xpd=TRUE,"Equilibrium\nevaluated",srt=90)
clip(x1 = 0,x2=12000,y1=-0.1,y2=1.04)
abline(v=10000,lty=2,lwd=2)

```

Of the `r length(courter.files)` replicates, `r length(unlist(lapply(s,nrow))[unlist(lapply(s,nrow))==10000])` reached an equilibrium by 10000 generations. 

```{r unlinked_courter_final}
final.freqs<-do.call(rbind,lapply(s,function(dat){
  last<-dat[nrow(dat),]
  return(last[,c("Generation","CourterFreq","CourterW","NonCourterW")])
}))
#aggregate the data
ws<-apply(final.freqs,1,function(rep){ return(c(rep["CourterW"],rep["NonCourterW"]))})
freqs<-apply(final.freqs,1,function(rep){ return(c(rep["CourterFreq"],1-rep["CourterFreq"]))})
eq<-apply(final.freqs,1,function(rep){ if(rep["Generation"]==9999) return("E") else return("N")})
#plot the final fitnesses
b<-barplot(ws,beside=TRUE,col=c(alpha(cols["courter"],0.75),alpha("darkgrey",0.75)),ylim=c(0,5),
        names.arg = rep("",10),ylab="Average Fitness",border = NA)
text(x=b[1,],y=1,label=freqs[1,],srt=90)
text(x=b[2,],y=1,label=freqs[2,],srt=90)
text(x=seq(2,30,3),y=-0.15,label=eq,xpd=T)
...
lapply(t,function(traits){
  ...
  gwsca.vioplot(males$LifetimeRS[males$Courter==1],males$LifetimeRS[males$Courter==0],
          col=c(cols["courter"],"darkgrey"),border=c(cols["courter"],"darkgrey"),ylim=c(0,40),plot.axes = FALSE)
...
All females nest with parental males, so the only way non-parental males reproduce is through sneaking. Parental males provide care that allows nests to have a 90% chance of survival. 
...
summary<-do.call(rbind,lapply(files,function(cat){
  dat<-do.call(rbind,lapply(cat,function(file){
    summ<-suppressWarnings(read.delim(file))
    if(nrow(summ) == 10000){
      last.freqs<-summ[nrow(summ),c("ParentFreq","CourterFreq","FreqNcNp", "FreqCNp", "FreqNcP", "FreqCP")]
    }else{
      last.freqs<-data.frame("ParentFreq"=-1,"CourterFreq"=-1,
                             "FreqNcNp"=-1, "FreqCNp"=-1, "FreqNcP"=-1, "FreqCP"=-1)
    }
    scen<-gsub("^(\\w+.*)_\\d+_summary.txt","\\1",file)
    last.freqs$scenario<-scen
    return(last.freqs)
  }))
  result<-as.data.frame(t(c(colMeans(dat[,1:6]),
                          unique(dat$scenario))))
  return(result)
}))
```
