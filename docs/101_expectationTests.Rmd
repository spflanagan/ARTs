...
Of the `r length(cu)` replicates, `r length(unlist(lapply(cu,nrow))[unlist(lapply(cu,nrow))==9999])` reached an equilibrium by 10000 generations. 
...
```{r courtParent_morphfreqs,echo=FALSE,out.extra='',fig.pos="H",fig.cap="Frequency of the 4 morphs in each rep",fig.width=6,fig.height=10}
...
Of the `r length(cl)` replicates, `r length(unlist(lapply(cl,nrow))[unlist(lapply(cl,nrow))==10000])` reached an equilibrium by 10000 generations.
...
<!-- ```{r courter_alleleFreqs_supergene, echo=FALSE,out.extra='',fig.height=6,fig.width=8,fig.pos="H",fig.cap="Supergene courter only--Major allele frequencies in the final generation of each replicate, with QTLs marked with purple stars"} -->
<!-- files<-list.files(pattern="^courter_\\d+_summary.txt") -->
...
<!-- ```{r, echo=FALSE,out.extra='',fig.height=6,fig.width=8,fig.pos="H",fig.cap="Supergene courter only -- Major allele frequencies in the final generation of each replicate, with qtls in green and marker loci in grey"} -->
<!-- plot.afs(maf,qtlcol=cols["courter"],prow=3,qtl.name="CourterQTL") -->
<!-- ``` -->
...
Variation was maintained in `r length(apply(final.freqs,1,function(r){ length(r[r==0])})[apply(final.freqs,1,function(r){ length(r[r==0])})<3])` of the `r nrow(final.freqs)`. `r length(unlist(lapply(ps,nrow))[lapply(ps,nrow) < 9999])` of the `r length(ps)` populations crashed, though. Of the `r length(ps)` replicates, `r length(unlist(lapply(ps,nrow))[unlist(lapply(ps,nrow))>=9999])` reached an equilibrium by 10000 generations.

<!-- ```{r parent_alleleFreqs_supergenes, echo=FALSE,out.extra='',fig.height=6,fig.width=8,fig.pos="H",fig.cap="Supergene parent only -- Major allele frequencies in the final generation of each replicate, with QTLs marked with purple stars"} -->
<!-- files<-list.files(pattern="^parent_supeergene_\\d+_summary.txt") -->

<!-- mycols <- colorRampPalette(c("grey",cols["parent"],"black"))(5) -->
<!-- maf<-plot.final.maf(files,mycols[c(2,4)],"ParentQTL",prow=3,smooth=TRUE) -->
<!-- ``` -->

<!-- ```{r, echo=FALSE,out.extra='',fig.height=6,fig.width=8,fig.pos="H",fig.cap="Supergenes parent only -- Major allele frequencies in the final generation of each replicate, with qtls in blue and marker loci in grey"} -->
<!-- plot.afs(maf,qtlcol=cols["parent"],prow=3,qtl.name="ParentQTL") -->
<!-- ``` -->
...
s<-plot.pc.reps("^parent-courter_supergene_[1-5]_summary.txt",cols)
```
...
<!-- ```{r courterParent_alleleFreqs_supergene, echo=FALSE,out.extra='',fig.pos="H",fig.height=6,fig.width=8,fig.cap="Supergenes -- Major allele frequencies in the final generation of each replicate, with courter QTLs marked with green stars and parent QTLs marked with blue stars"} -->
<!-- files<-list.files(pattern="^parent-courter_\\d+_summary.txt") -->
...
<!-- ```{r, echo=FALSE,out.extra='',fig.height=6,fig.width=8,fig.pos="H",fig.cap="Supergenes --Major allele frequencies in the final generation of each replicate, with courter qtls in green, parent qtls in blue, and marker loci in grey"} -->
<!-- plot.afs(maf,qtlcol=cols["courter"],prow=3,qtl.name="CourterQTL", -->
<!--          qtl.name2 = "ParentQTL",qtlcol2 = cols["parent"]) -->
<!-- ``` -->
