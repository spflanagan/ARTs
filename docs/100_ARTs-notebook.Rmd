...
So, evaluating the outputs from the simulation model, I'm not sure what's going on. First of all, the extent of diversity is really quite low in most cases, based on the expectations from the mathematical model -- only a few instances have substantial variation maintained, and most of the variation is quite low.

Somehow there are courter/non-parents surviving too, which is troubling, but could be due to the females also having the traits? Still, you'd expect them to be lost by the end of all the generations. I wonder if this is somehow related to the density dependence that we implemented to stop populations from crashing. 

But I've also realized that the `c` slider isn't working quite right in the app, so I'll need to fix that -- fixed! It seems as though the app might be constantly expecting diversity now, though, so maybe that's the problem -- perhaps I haven't solved the issue of the math at all. 

# 20 September 2021

Based on the new calculations, it looks like the updated mathematical model is suggesting that the sperm competition coefficient needs to be higher if we're going to have any diversity. So that's interesting. I'll try running those simulations along with the old baselines and see how they match up. 

# 17 September 2021

I've been brainstorming and taking notes outside of this document for the last week or so, but I'll detail some of the things I've found needed to be changed here:

- both the mathematical model and the simulation model had non-parents, not non-courters, be the sneakers, despite giving the non-courters the increased reproductive allocation and discounting them by c. So I've updated this.
- The simulation model had the nests survive before sneaking happened, which likely changed the fertilization patterns and costs of sneaking rather dramatically. I've now adjusted this to have all fertilization occur *before* nest survival is evaluated.
- the mathematical model determined the proportion of sneak-fertilized nests in a sort of strange way that didn't really make sense upon close examination, so I've re-done that.
- the final step in the mathematical model, evaluating $p_v$, included the morph frequency, but I don't think it should because the morph frequency is included in the first step and therefore doesn't need to be repeated. 

I've also adjusted my temrinology to remove the term 'sneaker' because I think that was causing me confusion. 

So now I'm re-running both the expectation_calculations.R and 100_single-locus.sh to see how these changes impact my initial results. Once I look at the results of expectation_calculations.R, I'll also re-run 101_model-informed-single-locus.sh.

I came across these inconsistencies by writing up the methods, so that's a really useful exercise! 

# 7 September 2021

I'm back to working on this. I found a region of parameter space taht *should* yield diverisity, based on the frequency-dependent mathematical model, but it doesn't necessarily in the simulations -- only with monogamy and only on occasion. It's very strange. It's also not that different from the low-diversity outcome. 

Maybe I didn't do the back-calculation from the reproductive parameter to the number for courters vs number for non courters. Baseline was r=2/3, which corresponded to courters getting 8/12 = 2/3 and non-courters getting 4/12 =1/3 of the nests. Diversity should be maximized ~ r = 1/3, so if I swap them it should work. What I actually ran was courters get 3/12 and non-courters get 10/12 (ok, numbers didn't add up), which should be r~1/4 and result in CS and NS in the population (no parents left). This is not what I'm seeing. 

I also haven't quite figured out how to get the brush-over table to appear in shiny. I made a bit of progress but it's not working right. I've changed the outputs so it shows the outcomes for equal starting conditions, and playing around a bit shows that 3/4 morphs should be maintained with r=0.4. This would be 0.4*12 ~ 5 for courters and ~7 for non-courters, so let's try running that. 

But first -- how do I get to a total of 12 eggs? This comes from the total number of eggs a male can receive, I think. I'll try running this but I'll want to check to see how this actually plays out in my simulation model -- it's possible the numbers I'm using don't relate to the $r$ value I think they do. 

# 6 Aug 2021

So the issue is that if I constrain both CP and NS initial frequencies, there's not enough parameter space to see patterns. But if I don't constrain both, then I've got another extra variable potentially playing a role in it...

Maybe add contour lines with the NS initial frequencies?

# 23 June 2021

I had reverted to using the paper notebook, but here I am switching back..

I created a shiny app to be able to interactively play with different parameters but it's SO SLOW to run over multiple generations AND multiple initial frequencies. 

I'm considering just running this across a list of various settings to see what happens. Alternatively, it looks like there's a bindCache() function that will store outputs -- this might be better. 

I'm also considering my interface -- perhaps it would be good/better to have some scroll bars for some of the key parameters as well, like sperm competition, selection coefficients, etc.


# 18 January 2021

I'm revisiting the document to get it ready to send to Suzanne, and have cleaned up some sloppy figure captions, labels, etc. 

Things to sort out today:
- what has happened with the summary file outputs, and can we see any patterns in terms of amount of sneaking?
- can I detect any patterns of initial conditions driving the outcomes?
- what might I suggest we discuss when I email her?

The random mating counts match up with what I expected to find for monogamous cases:


```{r}
courterRS1<-get.morph.freqs(plot.pc.reps(pattern="pcu_1locus_nestBinary_equalFitness.*summary.txt",path="single_locus",cols,make.plot=FALSE),gen="first")
courterRS<-get.morph.freqs(plot.pc.reps(pattern="pcu_1locus_nestBinary_equalFitness.*summary.txt",path="single_locus",cols,make.plot=FALSE))
summary(courterRS$NumRandMate)
summary(courterRS1$NumRandMate)
```

Looking at equivalent polgynyous cases, there doesn't seem to be any random mating that occurs.
...
I realize now that outputting the number randomly mated to stdout doesn't make life easy, and that I should instead save it to the summary file -- so I'll change that. In the meantime, though, I can sift through the log to see if I can make sense of anything. There are definitely some runs/generations where most/all females chose randomly and some where they did not -- it starts off with 0. So this is informative and will be more useful when paired with the info on the frequencies (i.e., when in the summary doc). 

# 28 December 2020

Now the model returns non-courter/parents as a dominant one in most cases of monogamy! I guess they win in sperm competition somehow? Or sneakers are able to be more successful at fathering offspring? This is a bit confusing to me. non-courter/parents would NOT be able to sneak fertilizations so they must be chosen through random mating. 

In polygyny, the courter/non-parents can survive because they can sneak fertilizations in nests belonging to parents. 

To try to sort this out a bit better, I'm re-running the model but tracking the number of random matings that occur.

# 22 December 2020 

Ok so the models have re-run. 

```{r}
traits<-read.delim("single_locus/pcu_1locus_nestBinary_nm_equalFitness_5_traits.txt")
traits[traits$Gen==12000 & traits$Courter==0 & traits$Parent==1 & traits$Sex=="MALE",]

```

Still the RS is 0...but one male has a lifetime RS of 35?!?! I guess that's because of the density dependence. But also the males are able to have multiple mates??

```{r}
traits<-read.delim("single_locus/pcu_1locus_polygyny_nm_nestBinary_courterRSadvantage_4_traits.txt")
traits[traits$Gen==12000 & traits$Courter==1 & traits$Parent==0 & traits$Sex=="MALE",]

```
Hmm the same thing is happening here. So I think maybe something's broken in the sneakers bit of code? I'll try to dig into it. 

One thing I've found is that I'm not actually enforcing the males-nesting-once rule when not in random mating. I also wasn't decreasing males' pot RS after sneaking. Ok, debuggin this, I'm finding that the max_sperm is getting HUGE, meaning that the fecundity share for each individual male is super small -- this is probably the problem. Why is this happening? because this idiot never initialized max_sperm. Now let's re-run those models and see what happens.

# 21 December 2020

Ok, so over the weekend I re-ran all the models, saving info on lifetime RS for sneaking males. Let's see what we can learn -- I've re-knitted the document, and it seems that re-running them has somewhat changed the outcomes. Let's take a look at one of the runs where non-courter/parents persist.

```{r}
traits<-read.delim("single_locus/pcu_1locus_nestBinary_onlySpermComp_2_traits.txt")
traits[traits$Gen==12000 & traits$Courter==0 & traits$Parent==1 & traits$Sex=="MALE",]
summary(traits$LifetimeRS[traits$Gen==0 & traits$Courter==0 & traits$Parent==1 & traits$Sex=="MALE"])
```

None of these males are able to produce offspring -- at any generation. Which means this is not driven by the reversion to random mating.

Next: look into the nm versions. And in those, there are indeed cases where non-courter/parents persist, which continues to surprise me. 

```{r}
traits<-read.delim("single_locus/pcu_1locus_nestBinary_nm_equalFitness_2_traits.txt")
traits[traits$Gen==12000 & traits$Courter==0 & traits$Parent==1 & traits$Sex=="MALE",]

```
Again, none of these males produce offspring -- do they produce offspring in gen 0?

```{r}
summary(traits$LifetimeRS[traits$Gen==0 & traits$Courter==0 & traits$Parent==1 & traits$Sex=="MALE"])
```

nope, as expected given the sneaker settings and non-mating settings. 

```{r}
traits[traits$Gen==12000 & traits$Courter==0 & traits$Parent==1 & traits$Sex=="FEMALE",]
```

The females produce fewer offspring than their allocation but do mate and produce offspring. Am I just not running the simulations for long enough? How close are the trait values to the threshold -- could mutations be flipping them over the edge?

```{r}
summary<-read.delim("single_locus/pcu_1locus_nestBinary_nm_equalFitness_2_summary.txt")
```

The parental threshold is fairly close to the mean trait value. I'm just not sure how NcP persists without NcNp also persisting. 

```{r}
par(mfrow=c(2,2))
lapply(unique(summary$Pop),function(pop){
  plot(summary$Generation[summary$Pop==pop],summary$FreqNcP[summary$Pop==pop],type="l")  
})

par(mfrow=c(2,2))
lapply(unique(summary$Pop),function(pop){
  plot(summary$Generation[summary$Pop==pop],summary$ParentAEmean[summary$Pop==pop],type="l")  
  abline(h=unique(summary$ParentThresh),lty=2,col="cornflowerblue")
})


par(mfrow=c(2,2))
lapply(unique(summary$Pop),function(pop){
  plot(summary$Generation[summary$Pop==pop],summary$CourterAEmean[summary$Pop==pop],type="l")  
  abline(h=unique(summary$CourterThresh),lty=2,col="cornflowerblue")
})
```

I'm running them with 20000 initial generations now -- we'll see if it makes any difference.

What about in the other runs where variation is maintained -- let's check to make sure those alternatives are maintained via males as we'd expect.

```{r}
traits<-read.delim("single_locus/pcu_1locus_nestBinary_sneakRSadvantage_2_traits.txt")
traits[traits$Gen==12000 & traits$Courter==1 & traits$Parent==0 & traits$Sex=="MALE",]

```

One interesting observation is that many of the individuals in generation 0 have a lifteime RS of 0, including females (this is because of no random mating, presumably). Huh, but in the random mating version females have a lifetime RS of 0. Oh, I guess if they randomly mate with a non-parent male.

Hmm the courter/non-parent morph doesn't seem to have a lifetime RS >0 either. Many of them found mates -- I guess maybe they used up their sperm on their own nests? 

```{r}
traits<-read.delim("single_locus/pcu_1locus_nestBinary_sneakRSadvantage_courterRSadvantage_4_traits.txt")
traits[traits$Gen==12000 & traits$Courter==0 & traits$Parent==0 & traits$Sex=="MALE",]

```

Ok, and this is true for ALL of the male morphs that are retained. So something odd seems to be going on. 

it turns out I did a bad job tracking sneaking and assigned all the fecundity shares to the parental male. So I've fixed the code and re-started the runs, but not with 20000 initial generations. We'll see where we get with the defaults. 

# 18 December 2020

I've tweaked the code to track sneaks as lifetime_rs, we'll see what the outputs of that look like -- in order to figure out how non-courters/parents persist, we need to determine whether they sneak or not.


# 17 December 2020

I tweaked the code but initial female frequencies don't seem to be massively important

```{r}
femFreqs<-get_female_freqs(pattern="pcu_1locus_nestBinary_nm_courterRSadvantage.*traits.txt",path="single_locus")
courterRS<-get.morph.freqs(plot.pc.reps(pattern="pcu_1locus_nestBinary_nm_courterRSadvantage.*summary.txt",path="single_locus",cols,make.plot=FALSE))
par(mfrow=c(3,1))
bp<-barplot(t(as.matrix(courterRS[,c("FreqNcNp", "FreqCNp", "FreqNcP")])),
        ...
        las=2,
        border=NA)
legend("topleft",bty='n',legend = c("Courter/Non-parent","Non-courter/Parent","Non-courter/Non-parent"),
       col=cols2[c("CNP","NCP","NCNP")],pch=15,xpd = TRUE,ncol=3,cex=1.25)

bp<-barplot(t(as.matrix(femFreqs[femFreqs$gen==0,c("NcNp", "CNp", "NcP","CP")])),
        col=cols2[c("NCNP","CNP","NCP","CP")],
        names.arg =  femFreqs$rep[femFreqs$gen==0],
        las=2,
        border=NA)

# barplot of just initial female frequencies
bp<-barplot(t(as.matrix(femFreqs[femFreqs$gen==0,c("Courter", "Parent")])),
        col=cols[c("courter","parent")],
        names.arg = femFreqs$rep[femFreqs$gen==0],
        las=2,
        border=NA)
abline(h=0.5,lty=2)
legend(y=1.2,x=15,xpd=TRUE,c("courter","parent"),col=cols[c("courter","parent")],
       ncol=2,pch=15,bty='n',cex=1.25)
```

When there are slightly more parents to start off with, the courter/non-parent is benefitted the most. This morph is able to sneak fertilizations and because it's a courter has a higher number of eggs -- so it can benefit. The non-courter/parent cannot sneak, so the only way that morph is retained is through females -- I'm still not sure how this one is maintained in these no-mating alternative scenarios! Mutation?

And what causes the differences among runs? It's obviously something to do with the initial conditions.

Let's look at the non-courter/parents:
```{r}
 traits[traits$Gen==12000 & traits$Courter==0 & traits$Parent==1,]
```
       Gen Pop Individual  Sex Courter CourtTrait Parent ParentTrait Preference PrefTrait MateFound PotRS LifetimeRS Alive
5000 12000   0        999 MALE       0    1.18571      1    0.441494          1         0         0     8          0     1
7000 12000   2        999 MALE       0    1.18571      1    0.441494          1         0         0     8          0     1
8000 12000   3        999 MALE       0    1.18571      1    0.441494          1         0         0     8          0     1


Hmm these ones are males...they did not mate -- but did they sneak? perhaps I should track/add that to the output, which could help me follow what's going on.

# 16 December 2020

So at the end of my work yesterday, the importance of female initial traits seemed to be important. 
```{r}
get_female_freqs<-function(pattern, path){
  files<-list.files(pattern=pattern,path=path,full.names = TRUE)
  femFreqs<-do.call(rbind,lapply(files, function(filename){
    traits<-read.delim(filename)
    allFreqs<-do.call(rbind, lapply(unique(traits$Pop),function (popindex, traits){
      traitsF<-traits[traits$Gen==0 & traits$Pop==popindex,]
      traitsL<-traits[traits$Gen==12000 & traits$Pop==popindex,]
      
      nums<-data.frame(NcNp=c(nrow(traitsF[traitsF$Sex=="FEMALE" & traitsF$Courter==0 & traitsF$Parent==0,]),
                                nrow(traitsL[traitsL$Sex=="FEMALE" & traitsL$Courter==0 & traitsL$Parent==0,])),
                         NcP=c(nrow(traitsF[traitsF$Sex=="FEMALE" & traitsF$Courter==0 & traitsF$Parent==1,]),
                                nrow(traitsL[traitsL$Sex=="FEMALE" & traitsL$Courter==0 & traitsL$Parent==1,])),
                         CNp=c(nrow(traitsF[traitsF$Sex=="FEMALE" & traitsF$Courter==1 & traitsF$Parent==0,]),
                                nrow(traitsL[traitsL$Sex=="FEMALE" & traitsL$Courter==1 & traitsL$Parent==0,])),
                         CP=c(nrow(traitsF[traitsF$Sex=="FEMALE" & traitsF$Courter==1 & traitsF$Parent==1,]),
                                nrow(traitsL[traitsL$Sex=="FEMALE" & traitsL$Courter==1 & traitsL$Parent==1,])))
      freqs<-nums/rowSums(nums)
      freqs$Courter<-c(nrow(traitsF[traitsF$Sex=="FEMALE" & traitsF$Courter==1,])/nrow(traitsF[traitsF$Sex=="FEMALE",]),
                       nrow(traitsL[traitsL$Sex=="FEMALE" & traitsL$Courter==1 ,])/nrow(traitsL[traitsL$Sex=="FEMALE",]))

      freqs$Parent<-c(nrow(traitsF[traitsF$Sex=="FEMALE" & traitsF$Parent==1,])/nrow(traitsF[traitsF$Sex=="FEMALE",]),
                      nrow(traitsL[traitsL$Sex=="FEMALE" & traitsL$Parent==1,])/nrow(traitsL[traitsL$Sex=="FEMALE",]))
      freqs$gen<-c(0,12000)
      freqs$rep<-as.numeric(gsub("^.*_(\\d)_traits.txt","\\1",filename))
      freqs$pop<-popindex
      return(freqs)
    }, traits = traits))
    return(allFreqs)
...
femFreqs<-get_female_freqs(pattern="pcu_1locus_nestBinary_courterRSadvantage.*traits.txt",path="single_locus")
```

Now to try to make some barplots
...
Ok, apparently I didn't save the first generation of traits for most of the runs -- so I'm re-running them...ahh it turns out I only output gen 0 for pop 0! I've updated the code but need to stop all my processes and re-run all of my models. 
...
Yesterday's runs seem to have finished, let's check them out.
...
Ok so ^ this brings up an important issue, which is that I don't know how the non-courter/parents exist in the population! Ohh maybe through females? 

```{r}
traits<-read.delim("single_locus/pcu_1locus_nestBinary_nm_onlySpermComp_1_traits.txt")
...
Ok, so the females are able to mate and maintain these traits in the population -- that's how!


```{r}
randm_patts<-c("pcu_1locus_nestBinary_RM_courterRSadvantage.*summary.txt",
             "pcu_1locus_nestBinary_RM_equalFitness.*summary.txt",
             "pcu_1locus_nestBinary_RM_onlySpermComp.*summary.txt",
             "pcu_1locus_nestBinary_RM_sneakRSadvantage_\\d.*summary.txt",
             "pcu_1locus_nestBinary_RM_sneakRSadvantage_courterRSadvantage.*summary.txt",
             "pcu_1locus_polygyny_nestBinary_RM_courterRSadvantage.*summary.txt",
             "pcu_1locus_polygyny_nestBinary_RM_equalFitness.*summary.txt",
             "pcu_1locus_polygyny_nestBinary_RM_onlySpermComp.*summary.txt",
             "pcu_1locus_polygyny_nestBinary_RM_sneakRSadvantage_\\d.*summary.txt",
             "pcu_1locus_polygyny_nestBinary_RM_sneakRSadvantage_courterRSadvantage.*summary.txt")
par(mfrow=c(2,length(randm_patts)/2),oma=c(3,1,4,1),mar=c(3,1,4,1),xpd=TRUE)
randDat<-lapply(randm_patts,function(pattern){
  dat<-get.morph.freqs(plot.pc.reps(pattern=pattern,path="single_locus",cols,make.plot=FALSE))
  bp<-barplot(t(as.matrix(dat[,c("FreqNcNp", "FreqCNp", "FreqNcP", "FreqCP")])),
        col=cols2[c("NCNP","CNP","NCP","CP")],
        names.arg =gsub("^.*(\\d)_summary.txt.*$","\\1",rownames(dat)),
...
The fact that random mating is not substantially different from the other cases indicates to me that the fitness differentials must not be large enough, or something else needs to change because random processes perform just as well as our so-called deterministic one. 


So these things suggest that:
- selection differentials aren't strong enough
- the model needs to run for a longer amount of time

Could the initial frequencies of the morphs *in females* be important in determining the final outcomes?

```{r}
levels(traits$Parent)<-c(0,1)
traits0f<-traits[traits$Gen==0 & traits$Pop==0,]
traits0l<-traits[traits$Gen==12000 & traits$Pop==0,]

nums0<-data.frame(NcNp=c(nrow(traits0f[traits0f$Sex=="FEMALE" & traits0f$Courter==0 & traits0f$Parent==0,]),
                          nrow(traits0l[traits0l$Sex=="FEMALE" & traits0l$Courter==0 & traits0l$Parent==0,])),
                   NcP=c(nrow(traits0f[traits0f$Sex=="FEMALE" & traits0f$Courter==0 & traits0f$Parent==1,]),
                          nrow(traits0l[traits0l$Sex=="FEMALE" & traits0l$Courter==0 & traits0l$Parent==1,])),
                   CNp=c(nrow(traits0f[traits0f$Sex=="FEMALE" & traits0f$Courter==1 & traits0f$Parent==0,]),
                          nrow(traits0l[traits0l$Sex=="FEMALE" & traits0l$Courter==1 & traits0l$Parent==0,])),
                   CP=c(nrow(traits0f[traits0f$Sex=="FEMALE" & traits0f$Courter==1 & traits0f$Parent==1,]),
                          nrow(traits0l[traits0l$Sex=="FEMALE" & traits0l$Courter==1 & traits0l$Parent==1,])))
freqs0<-nums0/rowSums(nums0)

```

Now I just need to do this for all of the pops so that I can make barplots like I Was doing yesterday.
