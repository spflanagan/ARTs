...
```
```{r}
ld_summary<-read.csv("../R/ld_summary_all.csv", header=FALSE)
colnames(ld_summary)<-c("CHR","POS","AvgR2","AvgR2Q","AvgR2NQ","file")

lds<-as.matrix(do.call(rbind,by(ld_summary,ld_summary$file, function(dat){
...
plot(lds[,"avgQ"]~lds[,"nchrom"])

plot(lds[,"avgResampled"]~lds[,"nchrom"])
```
...
```{r td}
qtl_info<-"../fixedART-results/supergene/highDiversity_qtls_monogamy_nm_prop0.05_q16_c2_1_qtlinfo.txt"
```
