...
```{r supergeneFig}
par(mfrow=c(6,1),mar=c(1,4,2,2),oma=c(2,2,1,1), xpd=TRUE)

chrIndex<-factor(gsub("(\\d)_\\d+","\\1",colnames(data2@geno)))
# GWAS courter
plot(data2@scores$CourtTrait$additive,
     ylab=expression(Courter~-log[10](p)),
     xlab="",
     xaxt='n',
     pch=19,
...
plot(data2@scores$ParentTrait$additive,
     ylab=expression(Parent~-log[10](p)),
     xlab="",
     xaxt='n',
     pch=19,
     col=c("grey73","grey44")[chrIndex],
...
```{r qtlExampleData, warning=FALSE}
# Looking into supergene haplotypes
...
par(mfrow=c(6,1),mar=c(1,4,2,2),oma=c(2,2,1,1))
...
plot(data2@scores$CourtTrait$additive,
     ...
     col=c("grey73","grey44")[chrIndex],
     bty='l')
courtQTLs<-qtl_dat[1,grep("CourterQTL",colnames(qtl_dat))]
qtllocs<-as.numeric(gsub("(\\d)\\.(\\d+)","\\1",courtQTLs))*1000 + 
  as.numeric(gsub("(\\d)\\.(\\d+)","\\2",courtQTLs))
points(qtllocs, 
       data2@scores$CourtTrait$additive[qtllocs],
...
plot(data2@scores$ParentTrait$additive,
     ...
     col=c("grey73","grey44")[chrIndex],
     bty='l')
parentQTLs<-qtl_dat[1,grep("ParentQTL",colnames(qtl_dat))]
qtllocs<-as.numeric(gsub("(\\d)\\.(\\d+)","\\1",parentQTLs))*1000 + 
  as.numeric(gsub("(\\d)\\.(\\d+)","\\2",parentQTLs))
points(qtllocs, 
       data2@scores$ParentTrait$additive[qtllocs],
       lwd=6,
       col=cols["parent"],
       cex=2)
...
plot(mtm_diff$Gprimest,pch=19,
     ...
     bty='l',
     col=c("grey73","grey44")[factor(mtm_diff$CHROM)])
points(qtl_locs, mtm_diff$Gprimest[qtl_locs],cex=2,col="red",lwd=2)
#abline(h=mean(mtm_diff$Gprimest, na.rm = TRUE),col="grey",lty=2,lwd=2)
#abline(h=median(mtm_diff$Gprimest, na.rm = TRUE),col="grey",lty=3,lwd=2)
...
lines(which(td$CHROM==1),
      ...
      col="grey44",
      lwd=2)
points(which(td$QTL==TRUE),
       td$TajimaD[td$QTL==TRUE],
       cex=2,
       col="red",
       lwd=2)
...
plot(mf_diff$Ht,
     ...
     ylab="Heterozygosity",
     xaxt='n')
points(qtl_locs, mf_diff$Ht[qtl_locs],cex=2,col="red",lwd=2)
