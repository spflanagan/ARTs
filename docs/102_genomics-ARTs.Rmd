...
following this one for better or worse: https://jendelman.github.io/GWASpoly/GWASpoly.html
...
I need to create the phenotype file

TODO: get the individual IDs to match up. Hmm traits file and vcf file have individuals in a different order. 
...
qq.plot(court_scan, trait="CourtTrait")
```

some departures from the null (dashed line) are present 


need to choose a -log10(p) threshold to control false positive rate. Bonferroni is overly conservative because LD means that markers are not independent. So we can use LD to estimate effective number of markers using method="M.eff", and can also use a permutation test.
...
Summarize it with actual qtls nearby
...
Now for the parent trait

```{r}



data.original <- set.K(data,LOCO=FALSE,n.core=2) # loco gave me an error in fit.ans
# test markers for significance
parent_scan <- GWASpoly(data=data.original,
                           models=c("additive","1-dom"),
...
qq.plot(parent_scan, trait="ParentTrait")
