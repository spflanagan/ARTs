...
LD is highest in low diversity scenarios and with QTLs -- this is surprising to me! I'd have thought supergenes would have the highest LD.
...
```{r eval=FALSE}
td_summary<-read.csv("../results/tajimaD_summary_Q1.csv", row.names = 1)
td_summary<-td_summary[grep("monogamy_nm",rownames(td_summary)),]
td_summary$rep<-gsub("../fixedART-results/.*\\/(.*\\w+_monogamy_nm_.*pop)_(\\d).*","\\1\\2",rownames(td_summary))
```
