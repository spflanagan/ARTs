...
We ran at least three sets of these identical replicates for all parameter combinations.
...
We evaluated whether QTLs evolved linkage disequilibrium (LD) due to shared
...
The purpose of exploring the baseline analytical model was to identify the
...
non-courter/non-parents), although the dominant morph depended on the specific
...
A striking difference between the analytical model and the simple simulation model is that only two morphs are maintained with parameters that retained three morphs in the analytical model (Fig. \@ref(fig:ternaryPlots)A; see Fig. S2 for outcomes with alternative mating dynamics). Furthermore, stochasticity played a major role in which
morph co-existed with the courter/parent morph, even when parameterization of the model was identical (Fig. S3).
Variation in outcomes within replicate runs can be attributed
...
### Polygenic architectures reduce effects of stochasticity, with minor impacts of supergenes
...
In the low diversity parameter space, the outcomes remained
...
stochasticity and recombination as opposed to balancing selection favouring multiple
morphs (Fig. S4). These results were consistent across all
...
\@ref(fig:ternaryPlots)B). 
Most simulations with supergenes resulted
...
frequencies, nor did the total number of QTLs underlying each locus. A consequence of the stabilisation of morph frequencies was a reduction in the strength of selection acting in the final generation relative to the first generation of the model (Fig. S5).
...
visualised the QTL haplotypes in supergenes (see Fig. S6 and S7 for representative images). These visualisations suggest that because of the
...
\@ref(fig:genomewideDiv) A; Fig. S8). In most runs, the heterozygosity at QTLs
...
\@ref(fig:genomewideDiv) B; Fig. S8). 
Average linkage disequilibrium was higher for QTLs in supergenes than randomly-distributed QTLs (Fig. S9).
...
significant $G_{ST}$ peaks, and found that under high-diversity parameters,
on average `r round(peak_summ_mean["qtls high"]*100,2)` $\pm$ `r round(peak_summ_sem["qtls high"]*100,2)`%, of randomly distributed QTLs and  and `r round(peak_summ_mean["supergene high"]*100,2)` $\pm$ `r round(peak_summ_sem["supergene high"]*100,2)`% of QTLs in supergenes were found in $G_{ST}$ peaks, irrespective of the courter/parent frequency (Fig.
S9).
...
A consequence of this high genetic diversity underpinning the categorical traits appears to be 
high error rates in detecting true QTLs using genome-wide association tests.
...
on fewer chromosomes (Fig. S10). 
The type of genetic architecture (genome-wide QTLs versus supergenes) had a minimal impact on the detection of QTLs. 
The QTLs for parent traits were more reliably detected by
GWAS than the courter QTLs, even though in the majority of simulations the final
population only contained parental males (Fig.
\@ref(fig:ternaryPlots)B).
The higher probability of detecting parent QTLs could be attributed to stronger
selection on that trait (i.e., being a non-parent was more costly than being a
non-courter).
...
the mechanisms shaping diversity, but we do not fully understand how necessarily polygenic phenotypes can be inherited as discrete suites of traits. 
Using a simulation model, we show that
genetic architecture -- specifically, the number of loci and the extent
of linkage among them -- impacts the evolution of complex
discrete phenotypes. In our single-gene models, under parameter
conditions that favoured the maintenance of multiple morphs in simple single-gene
...
The discovery of supergenes underlying polymorphism was hailed as an explanation for how complex suites of traits could be inherited in a discrete manner, when they must also be polygenic [@schwanderSupergenesComplexPhenotypes2014].
We provide compelling evidence that additive genetic variation need not be co-located in a non-recombining region of a chromosome, as long as the discrete traits are determined by a threshold model.
Real-world examples might differ in these assumptions; for example,the steroid hormone
receptor genes in white-throated sparrow and ruff supergenes
[@maneySupergenesSteroids2022] would be better approximated by a
logarithmic distribution than an additive distribution of allelic effects. 
Additionally, many polymorphisms could rely on
...
We modelled balancing selection on genomic regions primarily shared among members of
a population (morphs), which is analogous to sex-specific selection
[@morrisIntralocusTacticalConflict2013]. 
Unlike models of sex-specific
selection, which have found that under conditions resulting in balancing
selection on males and females genetic variation can be maintained
[@connallon2012], but that most genomic regions will not show excessive
variation [@flanaganIdentifyingSignaturesSexual2015], 
we observed both maintenance of variation in phenotypes and
in genotypes.
Additionally, we detected high levels of genetic differentiation between male types,
even without strong selection, which is opposite to the predictions for sex-specific selection
[@kasimatisLimitsGenomicDivergence2019]. 
As such, our model highlights
...
Furthermore, we expected the supergenes to evolve to contain single haplotypes for
...
observe discrete haplotypes underlying the categorical morphs. 
If we
...
models (Fig. S7). However, measures of genetic variation and
...
These unexpected levels of genetic diversity might have also impaired our ability to detect the causal QTLs using methods such as genome-wide association studies.
We only identified $\le$ 50% of true QTLs, with
