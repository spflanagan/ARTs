...
recombination due to inversions [@ford1965; @thompson2014at least three
species with alternative reproductive tactics: the ruff
...
imposed on displaying courting and/or parental care (following
<!--# find examples -->). In the baseline model, we multiplied the
viability selection coefficient (${w_v}_M$) by the surviving number of
offspring per morph ($o_M$), to determine the number of surviving
juveniles per morph: $j_M = o_M w_{v_M}$. In the simulation model, we
calculated a survival probability for each male offspring, as
...
maintained. Considering additional forms of complexity in genetic
architectures is a fruitful area for future research. For example, the
steroid hormone receptor genes in white-throated sparrow and ruff
supergenes [@maneySupergenesSteroids2022] would be better approximated
by a logarithmic distribution, which differs from the additive
distribution of allelic effects that we used. Additionally, many
