...
editor_options:
  ...
  markdown:
    wrap: 72
fontsize: 11pt
abstract: "Alternative reproductive tactics are an intriguing example of variation maintained within populations and have evolved independently in a wide variety of animals. When individuals cannot change their tactic throughout their life, genetic variants associated with the different morphs are expected to segregate in the population.  Empirical studies have found that in some cases these variants are clustered into  supergenes, whereas in other cases causal genetic variants are found throughout  the genome. An open question is to what extent the genetic architecture of loci  contributing to these reproductively important traits impacts the maintenance of  variation. Here, we present a model of a two-trait  male morph, in which males court (or do not court) females and provide parental care (or do not provide care). We first identify regions of parameter space in which balancing selection favours multiple male morphs in a genotype-free model, and used these  parameters in simulation models with quantitative trait loci (QTLs) contributing additively to each trait value, which is compared to a threshold to determine the categorical status for an individual. Using these explicit genetic simulations, we compared models with either a single genetic locus underpinning  the traits, many QTLs distributed throughout the genome, and QTLs co-located within  a low-recombination supergene. We found that adding explicit genetic architectures only enabled two morphs to be maintained, and that polygenic models reduce the impact of stochasticity compared to single-locus models, regardless of whether QTLs are co-located in a supergene. Genetic diversity was maintained even if phenotypic diversity was low, and attempts to identify true causal loci were unreliable, particularly for the highly polygenic traits. Together, these results suggest that supergenes are not necessary for the maintenance of multiple polygenic morphs, and that categorical traits with an threshold-based additive polygenic basis exhibit similar evolutionary dynamics to those determined by structural variants and by a single gene. \\par  quantitative trait locus, genome-wide genetic diversity  \\textbf{Keywords:} alternative mating tactics, frequency dependent selection, supergene"
preprint: no
mainfont: roboto
...
Each generation has three distinct phases: (1) mate choice; (2)
fertilization; (3) parental care; (4) viability selection; and (5)
maturation to adulthood (Fig. \@ref(fig:modelDiagram)). In the mate
choice phase of the model, we assume females always prefer to mate with
males displaying the courtship trait. If a female decides to mate with a
male, he receives all of her eggs to care for. The male is not
guaranteed paternity of his mates' entire brood, however, because
non-courting males are able to fertilize a proportion of the eggs, and
all males have an inherent reproductive allocation that sets a maximum
on the number of offspring he can sire (which is determined by his morph
phenotype). The chosen parent does receive preferred male fertilization
advantage. The consequence of this scenario is that the only mechanism
by which non-courters can produce offspring is through sneak
fertilizations. In the parental care phase of the model, we assume for
...
independent of the male's courtship trait. Finally, viability selection
is imposed on offspring before they mature to adulthood, such that males
carrying the courtship trait and/or the parental care trait have a lower
probability of survival than females and males without either courtship
or parenting. Males who are both courters and parents have a
...
We first explored a version of our model that could be solved
analytically to generate baseline predictions for when multiple morphs
are maintained. We then used an individual-based simulation model to ask
how the underlying genetic architecture of these traits affect (compared
to the baseline) the presence, frequency, and type of alternative male
phenotypes maintained in the population. We refer to these as the
baseline model and the simulation model respectively.

## Baseline model {#math-model-methods}

We developed a baseline analytical model that predicts the proportion of
morphs present in the population at each stage of the lifecycle (Fig.
\@ref(fig:modelDiagram)), without explicitly modelling genetic
properties. As such, it is a demographic model of the morphs, where
frequencies are altered from one generation to the next based on
selection at each life stage.

We first consider which morphs will be chosen by females (Stage 1 in
Fig. \@ref(fig:modelDiagram)), and how this corresponds to the total
number of clutches in the population. The maximum number of clutches is
determined by the probability of females mating with each type of male
morph: the probability of courting males (CP and CN) being chosen is
$w_s$ and the probability of NP and NN males being chosen is $1-w_s$.
...
where $N_m$ is the number of males in the population and the $f$ terms
represent the frequency of each morph. For simplicity, we focus on the
case where females only mate with courting males ($w_s=1$), so $n$
simplifies to $w_s(N_m f_{CP} + N_m f_{CN})$.

We next calculate the proportion of eggs in the population that are
sired by each morph (Stage 2 in Fig. \@ref(fig:modelDiagram)). Each
morph has an inherent maximum reproductive allocation, $r_M$ (where $M$
designates the morph and contains $\{CP, CN, NP, NN\}$). Chosen males
have the first priority to fertilize the eggs laid in their territory,
so the number of eggs fertilized within a single clutch per morph is
determined by the male reproductive effort of each morph and the
probability of being chosen: $e_{o_M} = f_M N_m r_M w_{s_M}$. However,
...
fertilizations arising from sneaking for a given non-courting male is
...
sneaking is smaller than the morph's potential population-level
reproductive effort (i.e, if $N_s < f_M N_m r_M$), the number of eggs
fertilized through sneaking for that morph is the difference between
$N_s$ and $f_M N_m r_M$, truncated at zero. Otherwise, the number of
eggs fertilized through sneaking is $f_M N_m r_M$.

Next, parental care determines whether clutches survive to a juveniles
(Stage 3 in Fig. \@ref(fig:modelDiagram)). The offspring surviving in
any male's clutch is dependent on whether he is a parent or not, which
determines the selection coefficient arising from clutch survival
($w_{n_M}$): $o_{o_M} = e_{o_M} w_{n_M}$. For simplicity, we consider
cases where $w_{n_CP} = w_{CN} = 1$ and $w_{n_NP} = w_{NN} = 0$. To
fully capture each morph's embryo survivorship, offspring that result
from sneaking (which depends on the frequency of other morphs and their
mating success) need to also be considered. We calculate the number of
offspring from sneaking that survive as:
...
the parental care trait.

Following the outcomes of parental care, we impose viability selection
on surviving juveniles (Stage 4 in Fig. \@ref(fig:modelDiagram)) by
multiplying the viability selection coefficient (${w_v}_M$) by the
surviving number of offspring per morph ($o_M$): $j_M = o_M w_{v_M}$.
Finally, these juveniles mature to adulthood (Stage 5 in Fig.
\@ref(fig:modelDiagram)) and we calculate the frequency of each morph in
the next generation by dividing the surviving number of juveniles
...
particular morphs, we tracked individuals within the population. Each
individual had a diploid genotype, which included both non-coding loci
and quantitative trait loci (QTLs). We included two types of QTLs: ones
that determined the individual's courting trait and ones that determined
the individual's parenting trait. Each allele at a QTL was assigned a
randomly-drawn effect on the phenotype, and an individual's quantitative
phenotypic value was determined by the sum of all their allelic effects
(i.e., traits were entirely additive). In this model, we assume no
environmental effects on the trait. Male traits (i.e.,
courter/non-courter and parent/non-parent) were determined by a
...
We considered three different genetic architectures for these QTLs: (a) a
single gene per trait without any additional genetic information (i.e., no non-coding loci to create chromosomes), (b)
multiple QTLs randomly placed throughout chromosomes, and (c) supergenes
located on one of multiple chromosomes. The single gene scenario is most
analogous to the baseline model described above. When the traits had
...
phases: (1) mate choice; (2) fertilization; (3) clutch survival; (4)
viability selection on juveniles; (5) maturation to adulthood (Fig.
...
acceptable mate, she did not mate (see supplement for less strict mating dynamics).
...
As part of the fertilization process, recombination occurred
independently on the chromosome inherited from the mother and the
...
[@lamichhaneyStructuralGenomicChanges2016; @kupperSupergeneDeterminesHighly2016]).

After recombination occurred, if the progeny survived, the genotypes
underwent mutation, which occurred at a rate of $\mu$ at each locus. For
a given individual, the number of mutations introduced was determined as
$2\mu n_{SNPs} n_{chrom}$, where $n_{SNPs}$ is the number of SNPs
and $n__{chrom}$ is the number of chromosomes in the simulation. The
...
To explore the impacts of genetic architecture on morph maintenance, we used two sets of demographic and reproductive parameters that were identified by the baseline model as either facilitating high or low morph diversity. Within each of these parameter settings, we compared model outcomes with for cases with each combination of 2, 4, or 8 chromosomes and 8, 16, 32, or 68 QTLs underlying each trait. We also tested three supergene sizes: occupying 5% of a chromosome, 25% of a chromosome, or 50% of a chromosome. In all cases, a chromosome was comprised of 1000 loci (including marker loci and QTLs). 


### Assessing model sensitivity and dynamics

We first compared the predictions of our baseline model to the results of a simulation model with a single locus for each trait in order to establish the effect of stochasticity on the predictions of the model. This allowed us to separate the effects of stochasticity inherent to a simulation from how the genetic architecture affects the maintenance of alternative male morphs. Using a consistent set of population parameters that determined the extent of balancing selection on the population, we then explored the impacts of different genetic architectures on the maintenance of male morphs.
...
male morphs, and explicit genetic architectures impacts which morphs are maintained under balancing selection. In all diagrams, each point in the triangle represents a different
...
different combinations of two out of the three morphs. (A) A comparison of the baseline analytical model and the architecture-free single-locus simulations. In these diagrams, point size reflects the number of parameter settings resulting in that exact combination of morph frequencies. The left panel represents the
predictions of the baseline model, in which most parameter combinations
result in the fixation of the courter/parent morph but many retain three morphs. The outcomes of the
...
architectures for the chosen low diversity parameters (top) and high
...
outcomes. The ternary diagrams show the final frequencies of the
courter/parent, non-courter/parent, and non-courter/non-parent
individuals from individual-based simulations with QTLs distributed
throughout the genome (left column) or clustered into supergenes that
make up 5%, 25%, or 50% of the chromosome the supergene is situated on.
Colours represent the number of chromosomes considered by the model and
the size of the points depict the number of QTLs underlying each trait.
In low diversity scenarios, regardless of genetic architecture, the
courter-parent morph is the one that is retained, same as in the
baseline model and architecture-free simulations in (A). Unlike in the
baseline and architecture-free models, in high diversity scenarios the
...
starting conditions are identical (Fig. \@ref(fig:ternaryPlots)A).
...
```{r genomewideDiv, fig.cap="Genome-wide genetic diversity in the final generation of the simulations was not influcenced by genetic architecture or the phenotypic diversity (i.e., morph frequencies), but did differ between low- and high-diversity parameter settings. Shown are genome-wide observed heterozygosity (A), Tajima’s D (B), and male-male Fst (C).", fig.keep=TRUE, fig.height=3.5}

par(mfrow=c(1,3),xpd=TRUE, mar=c(5,5,1.5,1), oma=c(2,2,5,1))
