...
```{r genomewideDiv, fig.cap="Genome-wide genetic diversity in the final generation of the simulations was not influcenced by genetic architecture or the phenotypic diversity (i.e., morph frequencies), but did differ between low- and high-diversity parameter settings. Shown are genome-wide observed heterozygosity (A), Tajima’s D (B), and male-male Fst (C) were not influenced by the frequency of the morphs in the final generation, nor by the genetic architecture.", fig.keep=TRUE}

par(mfrow=c(1,3),xpd=TRUE, mar=c(5,5,1.5,1), oma=c(2,2,3,1))
...
Our model can provide insight into expected levels of genetic diversity
when selection is action on genomic regions that are primarily shared
among members of a population, which can be analogous to sex-specific
selection [@morrisIntralocusTacticalConflict2013]
...
When it comes to morphs, this might not be true -- we observed both
maintenance of variation in phenotypes and in genotypes, with reasonably
high levels of genetic differentiation between male types, even after
selection had been relaxed. In other words, even the runs where the
...
from single-locus models [@kasimatisLimitsGenomicDivergence2019]. As
such, our model highlights how demographic factors, stochasticity, and
genomic effects such as pleiotropy and linkage can impact genetic
variation, and potentially obscure putative genome-wide signals of
selection.
...
-   combine the two diagrams into one

-   move table to supplement (focus on logic in main text)

-   combine two ternry diagrams -- add labels (baseline + simulation)
    and consistent shapes

-   genomewide figures:

    -   refine the first statement to better highlight the takehome

    -   only the panels a, c, e

-   read abstract and make sure they take homes

-   think about nuance and reviewer push back

    -   additive model with threshold

-   focus on abstract and short pithy take-home (superficial)
