...
with the subscript $M$), denoted by a sexual selection parameter $w_s$.
In general for this model, the probability of NP and NN males being
chosen should equal $1-w_s$, where $w_s$ is the probability of courting
males (CP and CN) being chosen. The general calculation for the number
of clutches is:
...
(\$n\_{sneakers}\$) that are allowed to fertilize each clutch. If the
number of fertilizations arising from sneaking is smaller than the
morph's potential reproductive effort (i.e, if \$N_s \\lt s_M\$), the
number of eggs fertilized through sneaking for that morph is the
difference between $N_s$ and \$s_M\$, truncated at zero. Otherwise, the
number of eggs fertilized through sneaking is $s_M$.
...
(\$w\_{n_M}\$), which for our purposes equals 1 for CP and NP males and
...
influenced by $n_{sneak}$, with larger contributions from an individual
non-courting male (e.g., larger $c$) being required with fewer sneakers
(Fig. \@ref(fig:mathParams)).
...
shift the parameter space in which polymorphism are stable, but can
...
outcomes than QTLs distributed genome-wide, and unexpectedly high levels
of genetic diversity were retained.

What drove our results?

-   LD, selection (and relaxation of selection?)

-   genetic correlations - compare to previous models and speculate on
    how it might impact their results - discuss real-world examples

-   a mention of intralocus conflict?

Our choice to compare QTLs co-located within non-recombining supergenes
to a genome-wide distribution was motivated by recent evidence of
alternative reproductive morphs having both of these genetic
architectures. A striking difference in our results from those of the
empirical ones -- in particular, the empirical results relating to
supergenes -- is that we do not see fixed alleles exclusively in one
morph of the other. Instead, quantitative variation is retained within
the supergenes, and we do not observe discrete haplotypes underlying the
categorical morphs. To some extent, this is likely an artefact of our
model set-up. In empirical examples, a key gene substantially
...
[@maneySupergenesSteroids2022]. In our model, we assumed all QTLs had
roughly similar but small contributions to the phenotype. Furthermore,
many polymorphisms could rely on dominance hierarchies (e.g., modelled
as such by X), and could even be underpinned by dominance reversals
between morphs [@pearseSexdependentDominanceMaintains2019]or sexes
...
[@boltonColourPolymorphismLikely2016].
...
[22]). We performed these analyses on simulated genome-wide data, and
found that genome-wide association studies are likely to only identify
$\le$ 50% of true QTLs, and identify many false positives. Low true
positives and high false positives are consistent with other simulation
studies of genome-wide studies of selection and trait associations
...
point when the populations were 'genotyped' -- mutations could have
cropped up that were no longer being removed by selection due to relaxed
selection.

Recent mathematical and simulation models have helped clarify the
allelic variation that is expected to be maintained under sex-specific
selection pressures, and importantly have suggested that sex-specific
selection must be incredibly strong
...
phenotypes and in genotypes, with high-ish levels of genetic
differentiation between male types.

-   genetic architecture is critically important

-   this doesn't include other elements of sex-specific or
    morph-specific architectures such as sex-specific dominance of
    sex-biased gene expression

This model only applies to fixed polymorphisms, which only describes a
portion of alternative reproductive tactics

-   hormonal control/cascades

-   gene expression networks and dynamics
