...
influence the maintenance or loss of morphs using an analytical model.
...
Each generation has five distinct phases: (1) mate choice; (2)
...
properties. As such, this model implicitly assumed a haploid population
where morphs are determined by a single locus, with no mutation,
recombination, or environmental effects.

In the first stage, morphs were chosen as mates by females (Stage 1 in
Fig. \@ref(fig:modelDiagram)), which determined the total number of
clutches in the population. The maximum number of clutches was
determined by the probability of females mating with each type of male
morph: the probability of courting males (CP and CN) being chosen was
$w_s$ and the probability of NP and NN males being chosen was $1-w_s$.
...
where $N_m$ was the number of males in the population and the $f$ terms
represented the frequency of each morph. For simplicity, we focussed on
the case where females only mated with courting males ($w_s=1$), so $n$
simplified to $w_s(N_m f_{CP} + N_m f_{CN})$.

We next calculated the proportion of eggs in the population that were
sired by each morph (Stage 2 in Fig. \@ref(fig:modelDiagram)). Each
morph had an inherent maximum reproductive allocation, $r_M$ (where $M$
designated the morph and contained $\{CP, CN, NP, NN\}$). Chosen males
had the first priority to fertilize the eggs laid in their territory, so
the number of eggs fertilized within a single clutch per morph was
determined by the male reproductive effort of each morph ($e_{o_M}$) and
the probability of being chosen: $e_{o_M} = f_M N_m r_M w_{s_M}$.
However, non-courting males could also sneak fertilizations, which
impacted the overall proportion of fertilizations per morph. The maximum
number of possible fertilizations arising from sneaking for a given
non-courting male was $N_s = n c n_{sneakers} r_{NN}$; for every clutch
($n$), each sneaker could contribute up to their individual reproductive
allocation (where $r_{NN} = r_{NP}$) offset by the sperm competition
coefficient ($c$), but this potential allocation was proportional to the
number of sneakers ($n_{sneakers}$) allowed to fertilize each clutch. If
the number of fertilizations arising from sneaking was smaller than the
...
Next, parental care determined whether clutches survived to become
juveniles (Stage 3 in Fig. \@ref(fig:modelDiagram)). The offspring
surviving in any male's clutch was dependent on whether he was a parent
or not, which determined the selection coefficient arising from clutch
survival ($w_{n_M}$): $o_{o_M} = e_{o_M} w_{n_M}$. For simplicity, we
considered cases where $w_{n_CP} = w_{CN} = 1$ and
$w_{n_NP} = w_{NN} = 0$. To fully capture each morph's offspring
survivorship, offspring that resulted from sneaking (which depended on
the frequency of other morphs and their mating success) needed to also
be considered. We therefore calculated the number of offspring from
sneaking that survive as:
...
Following the outcomes of parental care, we imposed viability selection
...
Finally, these juveniles matured to adulthood (Stage 5 in Fig.
\@ref(fig:modelDiagram)) and we calculated the frequency of each morph
in the next generation by dividing the surviving number of juveniles
...
Note that this model did not incorporate mutation, recombination, or inheritance
via females. Nonetheless, this model provided a baseline predictions for
parameter values likely to facilitate morph diversity within a
population. We relaxed these assumptions in the individual-based
simulation model.
...
simulation model. The simulation model followed a lifecycle that is
...
a single gene per trait without any additional genetic information
(i.e., no non-coding loci to create chromosomes), (b) multiple QTLs
randomly placed throughout chromosomes, and (c) supergenes located on
one of multiple chromosomes. The single gene scenario is most analogous
to the baseline model described above. When the traits had multiple QTLs
determining their trait values, each QTL was assigned a location on a
chromosome and the QTL's effect on the trait is associated with a single
nucleotide polymorphism (SNP). These SNPs underwent recombination and
...
we assumed all QTLs are autosomal, and we did not model sex chromosomes.
...
Each generation included five stages, same as the baseline model: (1)
mate choice; (2) fertilization; (3) clutch survival; (4) viability
selection on juveniles; (5) maturation to adulthood (Fig.
\@ref(fig:modelDiagram)). The main differences between the
individual-based simulation model and the baseline model were the
introduction of stochasticity during mate choice and survival and the
introduction of recombination and mutation. During the mate choice
...
acceptable mate, she did not mate (see supplement for less strict mating
dynamics).
