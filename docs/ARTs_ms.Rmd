...
properties. As such, this model implicitly assumes a haploid population where morphs are determined by a single locus, with no mutation, recombination, or environmental effects.
...
parameter space in which, in a simplistic model, more than one male
...
and the number of sneakers allowed ($n_{sneak}$).
...
$0.5 \le r \le 1$. As an example of why this makes sense, if
...
male (e.g., larger $c$) being required to maintain diversity of morphs when fewer sneakers were allowed (Fig. S1).
