...
When just the parent trait is present, regardless of the parameter settings or genetic architectures, the parental male morph goes to fixation in every run (Table \@ref(tab:parentSummTable)).
...
The most interesting patterns are expected to occur in the two-trait model, when males have both a courter and a parenting trait. This next section looks at the impact of tradeoffs in the reproductive allocation (courting vs non-courting) and parental care given our three different inheritance mechanisms to see when multiple morphs are maintained in the populations.
...
(ref:plotRAunlinkedSumm) Impacts of reproductive allocation and nest survival on the maintenance of multiple male morphs given a completely unlinked genetic architecture underlying the traits. Along the x-axis are ratios of the ratio of courter to non-courter reproductive allocation (i.e., effort exerted pre-nesting) and on the y-axis are ratios of parent to non-parent nest survival. Each combination of settings shows multiple runs (in no particular order), each in its own stacked barplot, with different colors representing the various morphs. 

```{r plotRAunlinkedSumm, fig.cap="(ref:plotRAunlinkedSumm)"}
plot_2vars_summary(cpu_all_freqs,"Courter2NonRS","Parent2NonSurvival",mar=c(5.1,4.1,4.1,2.1))
```

```{r}
cpu_var<-cpu_all_freqs[which(round(cpu_all_freqs$Parent2NonSurvival,1)==3.5 & cpu_all_freqs$Courter2NonRS==0.5),]
cpu_var_init<-get.morph.freqs(plot.pc.reps(pattern=unique(gsub("(.*)\\/(.*)\\_\\d$","\\2",rownames(cpu_var))),
                                           path=unique(gsub("(.*)\\/(.*)\\_\\d$","\\1",rownames(cpu_var))),
...
(ref:plotRAlinkedSumm) Impacts of reproductive allocation and nest survival on the maintenance of multiple male morphs given QTLs underlying the traits being anchored to chromosomes. Along the x-axis are ratios of the ratio of courter to non-courter reproductive allocation (i.e., effort exerted pre-nesting) and on the y-axis are ratios of parent to non-parent nest survival. Each combination of settings shows multiple runs (in no particular order), each in its own stacked barplot, with different colors representing the various morphs. 

```{r plotRAlinkedSumm, fig.cap="(ref:plotRAlinkedSumm)"}
...
(ref:plotRAsupergeneSumm) Impacts of reproductive allocation and nest survival on the maintenance of multiple male morphs given QTLs being located in one chromosomal region (in a supergene). Along the x-axis are ratios of the ratio of courter to non-courter reproductive allocation (i.e., effort exerted pre-nesting) and on the y-axis are ratios of parent to non-parent nest survival. Each combination of settings shows multiple runs (in no particular order), each in its own stacked barplot, with different colors representing the various morphs. 


```{r plotRAsupergeneSumm, fig.cap="(ref:plotRAsupergeneSumm) "}
...
By 'mating system' I refer to whether males are able to mate with multiple females (i.e., be the nesting male for multiple females). The default is monogamy, meaning males can only be the nesting male to one female. In these results, I'm showing the mating system as a function of the threshold values, which determine the trait value for which a male becomes a courter or non-courter and a parent or non-parent. These thresholds are set to the mean
...
cpu_ms_freqs<-rbind(cpu_polygyny_freqs[,poly_keep_cols],cpu_baseline_freqs[,poly_keep_cols])
cpu_ms_freqs$MSnum<-as.numeric(cpu_ms_freqs$Polygyny)
```

(ref:unlinkedPolygynyBarplot) A stacked barplot showing each run of the model with unanchored QTLs, with polygyny (labelled 1 on the x-axis) or with monogamy (labelled 0 on the x-axis). Colors represent different morphs.

```{r unlinkedPolygynyBarplot, fig.cap="(ref:linkedPolygynyBarplot)",fig.height=4,fig.width=6}
bp<-barplot(t(as.matrix(cpu_ms_freqs[,c("FreqNcNp", "FreqCNp", "FreqNcP", "FreqCP")])),
        col=cols2[c("NCNP","CNP","NCP","CP")],
        names.arg = cpu_ms_freqs$MSnum,
        las=2,
        border=NA)
abline(v=max(bp[which(cpu_ms_freqs$Polygyny==TRUE)])+0.5,lwd=2)
...
cpl_ms_freqs<-rbind(cpl_polygyny_freqs[,poly_keep_cols],cpl_baseline_freqs[,poly_keep_cols])
cpl_ms_freqs$MSnum<-as.numeric(cpl_ms_freqs$Polygyny)
```

(ref:linkedPolygynyBarplot) A stacked barplot showing each run of the model with QTLs linked to chromosomes, with polygyny (labelled 1 on the x-axis) or with monogamy (labelled 0 on the x-axis). Colors represent different morphs.

```{r linkedPolygynyBarplot, fig.cap="(ref:linkedPolygynyBarplot)",fig.height=4,fig.width=6}
bp<-barplot(t(as.matrix(cpl_ms_freqs[,c("FreqNcNp", "FreqCNp", "FreqNcP", "FreqCP")])),
        col=cols2[c("NCNP","CNP","NCP","CP")],
        names.arg = cpl_ms_freqs$MSnum,
        las=2,
        border=NA)
abline(v=max(bp[which(cpl_ms_freqs$Polygyny==TRUE)])+0.5,lwd=2)
...
cps_ms_freqs<-rbind(cps_polygyny_freqs[,poly_keep_cols],cps_baseline_freqs[,poly_keep_cols])
cps_ms_freqs$MSnum<-as.numeric(cps_ms_freqs$Polygyny)
```

(ref:supergenePolygynyBarplot) A stacked barplot showing each run of the model with QTLs in supergenes, with polygyny (labelled 1 on the x-axis) or with monogamy (labelled 0 on the x-axis). Colors represent different morphs.

```{r supergenePolygynyBarplot, fig.cap="(ref:supergenePolygynyBarplot)",fig.height=4,fig.width=6}
bp<-barplot(t(as.matrix(cps_ms_freqs[,c("FreqNcNp", "FreqCNp", "FreqNcP", "FreqCP")])),
        col=cols2[c("NCNP","CNP","NCP","CP")],
        names.arg = cps_ms_freqs$MSnum,
        las=2,
        border=NA)
abline(v=max(bp[which(cps_ms_freqs$Polygyny==TRUE)])+0.5,lwd=2)
...
One of the settings in the model is the proportion of a chromosome that can contain the supergene -- this controls how tightly linked the SNPs are, given that they all occur in the same region of the same chromosome. Larger values indicate less tight linkage, since the QTLs are spaced across a larger proportion of the genome.

```{r supergeneProp}
cs_props<-get.morph.freqs(plot.courter.reps(pattern="^courter_supergene_prop.*summary.txt.gz",path="sensitivity",cols,make.plot=FALSE))
ps_props<-get.morph.freqs(plot.parent.reps(pattern="^parent_supergene_prop.*summary.txt.gz",path="sensitivity",cols,make.plot=FALSE))
```

(ref:PCsupergeneProp) .

```{r PCsupergeneProp, fig.cap="(ref:PCsupergeneProp)",fig.width=6,fig.height=4}
pc_props<-get.morph.freqs(plot.pc.reps(pattern="^parent-courter_supergene_prop.*summary.txt.gz",path="sensitivity",cols,make.plot=FALSE))
pc_props$Proportion<-as.numeric(gsub(".*prop(.*)_\\d+_summary.txt.gz.*","\\1",rownames(pc_props)))
bp<-barplot(t(as.matrix(pc_props[,c("FreqNcNp", "FreqCNp", "FreqNcP", "FreqCP")])),
        col=cols2[c("NCNP","CNP","NCP","CP")],
        names.arg = pc_props$Proportion,
        las=2,
        border=NA)
l<-lapply(unique(pc_props$Proportion),function(prop){
  abline(v=max(bp[which(pc_props$Proportion==prop)])+0.5,lwd=2)
})
par(fig=c(0, 1, 0, 1), oma=c(0, 0, 0, 0), mar=c(0, 0, 0, 0), new=TRUE)
plot(0,0, type='n', bty='n', xaxt='n', yaxt='n')
legend("top",bty='n',legend = c("Courter/Parent","Courter/Non-parent","Non-courter/Parent","Non-courter/Non-parent"),
