...
plot(cng.pop$PopSize,lwd=2,xlab="Generations",ylab="PopSize",
     bty="L",type="l")
...
plot(pcng.sum$ParentW,lwd=2,xlab="Generations",ylab="Parental Reproductive Success",
     bty="u",type="l",ylim = c(0,4.25),col=alpha(cols["parent"],0.75),las=1)
...
plot(pcng.sum$CourterW,xlab="Generations",ylab="Courter Reproductive Success",
     bty="u",lwd=2,type="l",col=alpha(cols["courter"],0.75),ylim = c(0,4.25))
...
plot(jitter(pcng.traits$Courter,1.5),jitter(pcng.traits$Parent,1.5),
     cex=((pcng.traits$LifetimeRS/max(pcng.traits$LifetimeRS))+1),
     axes=F,xlab="",ylab="",pch=19,col=alpha(cols2[pcng.traits$Morph],0.75),lwd=2)
text(x=c(0,1),y=-0.4,c("Non-Courter","Courter"),xpd=TRUE)
text(x=-0.4,y=c(0,1),c("Non-Parent","Parent"),xpd=TRUE,srt=90)

##How can I create a fitness landscape?? 
##I have discrete values
library(lattice)
contourplot(pcng.traits$LifetimeRS ~ pcng.traits$CourtTriat*pcng.traits$ParentTrait,
  xlab = "Courter", ylab = "Parent",
  main = "Reproductive Success",
  col.regions = rscol,
  panel=function(...){
    panel.levelplot(...)
    panel.abline(h=pcng.sum$ParentThresh,lwd=2)
    panel.abline(v=pcng.sum$CourterThresh,lwd=2)
  }
)#not very informative


levelplot(pcng.traits$LifetimeRS ~ pcng.traits$Courter*pcng.traits$Parent,
  xlab = "Courter", ylab = "Parent",
  main = "Reproductive Success",
  col.regions = rscol
)

heatmap(pcng.traits$LifetimeRS ~ pcng.traits$CourtTriat*pcng.traits$ParentTrait)

image(pcng.traits$LifetimeRS,col = rscol)
