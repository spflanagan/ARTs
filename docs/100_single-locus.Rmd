...
bp<-barplot(t(as.matrix(courterRS[,c("FreqNcNp", "FreqCNp", "FreqNcP", "FreqCP")])),
        ...
        names.arg = courterRS$Generation,
        las=2,
        border=NA)
...
Some of these crashed (ones with generation 0), presumably becuase all of the non-courters were given nests.
...
bp<-barplot(t(as.matrix(courterRSpoly[,c("FreqNcNp", "FreqCNp", "FreqNcP", "FreqCP")])),
        col=cols2[c("NCNP","CNP","NCP","CP")],
        names.arg = courterRSpoly$Generation,
        las=2,
        border=NA)
...
Somehow it seems like the seed isn't being set properly, because the polygyny and non-polygyny have the same thresholds! And the same thing happened here. 
...
par(mfrow=c(2,length(var_patts)/2))
...
plot(0,0, type='n', bty='n', xaxt='n', yaxt='n')
legend("top",bty='n',legend = c("Courter/Parent","Courter/Non-parent","Non-courter/Parent","Non-courter/Non-parent"),
       col=cols2[c("CP","CNP","NCP","NCNP")],pch=15,xpd = TRUE,ncol=4,cex=1.25)

```

I'm concerned about this non-independence thing. 