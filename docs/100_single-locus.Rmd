...
The first of these scenarios arise when courters and parents have a reproductive advantage (they can fertilize 8 eggs compared to the non-courter and non-parent's 4 eggs). The nesting male (regardless of phenotype) experienced a reproductive advantage by fertilizing a higher proportion of the eggs. Furthermore, females always prefer courters to non-courters and only nest with non-courters when they cannot find a courter in the males they evaluate. 
...
```{r courterRSinitialvfinal,fig.cap="The influence of initial frequency of males with the parental trait on the final frequency of the parental trait in the population. The shapes represent independent rounds of sets of 4 replicates.",fig.height=4,fig.width=4}
...
plot(courterRS1$ParentFreq,courterRS$ParentFreq,
     pch=as.numeric(gsub("^.*(\\d)_summary.txt.*$","\\1",rownames(courterRS))),
     xlab="Initial Parent Frequency",
     ylab="Final Parent Frequency",
     cex=3,
     xlim=c(0.46,0.52),
     ylim=c(0.95,1.01))
legend("bottomleft",pch=unique(as.numeric(gsub("^.*(\\d)_summary.txt.*$","\\1",rownames(courterRS)))),
       unique(gsub("^.*(\\d)_summary.txt.*$","\\1",rownames(courterRS))))
```

However, when males are allowed to mate multiply, the courter/parent morph dominates the population but is not always the only morph present (Fig. \@ref(fig:polyCourterRS)).
...
1. What causes one or another sneaker morph to become present in the population?

2. How do non-intuitive morphs like the non-courter/parent persist, when they shouldn't mate at all?

3. What causes the variation among morphs?



The answers to these three questions seem to be somewhat linked. First, I dug into the results to look into the relationship with initial frequencies to see how important those stochasitic differences in morph frequencies are in the outcomes. 



```{r}
courterRS<-get.morph.freqs(plot.pc.reps(pattern="pcu_1locus_polygyny_nestBinary_courterRSadvantage.*summary.txt",path="single_locus",cols,make.plot=FALSE))
courterRS1<-get.morph.freqs(plot.pc.reps(pattern="pcu_1locus_polygyny_nestBinary_courterRSadvantage.*summary.txt",path="single_locus",cols,make.plot=FALSE),gen = "first")
```
```{r}
par(mfrow=c(2,1))
