...
That being said, every time a male mates he uses his sperm, so after one mating where a courter fertilizes 50% of the female's 4 eggs, he only has $n_{sperm}-2$ sperm for his next mating. 
...
```{r eqFitness, fig.cap="A close-up view of the equal fitness pane from above, showing the final frequencies of the four morphs for the polygyny, equal fitness, no random mating runs of the model.",fig.height=3,fig.width=6.5}
