...
```{r lowDivQTLs1, fig.cap="Final generation frequencies based on the model with 1 QTLs, with parameters that are expected to result in no diversity based on the mathematical model. In 'monogamy' cases, the males were constrained to only accept eggs from one female into their nests, whereas under 'polygyny' males could mate with multiple females. In both cases females mated once. Parameter settings lablled with 'nm' were runs where females did not mate if they could not find a suitable mate (i.e., if no courters were available)."}
...
var_patts<-unique(gsub(".*lowDiversity_(.*)_\\d_summary.txt_\\d","\\1",
                       lnames[grep(paste0("q",qs[5]),lnames)]))
...
```{r lowDivQTL1 super, fig.cap="Final generation frequencies based on the model with 1 QTLs in a supergene, with parameters that are expected to result in no diversity based on the mathematical model. In 'monogamy' cases, the males were constrained to only accept eggs from one female into their nests, whereas under 'polygyny' males could mate with multiple females. In both cases females mated once. Parameter settings lablled with 'nm' were runs where females did not mate if they could not find a suitable mate (i.e., if no courters were available)."}
...
```{r highDivGenetics}
highDiv<-get.morph.freqs(plot.pc.reps(pattern="highDiversity.*monogamy_nm.*summary.txt",
                                     path="/mnt/BigData/fixedART-results/qtls",
                                     cols,make.plot=FALSE))
hnames<-rownames(highDiv)
```

```{r}
qs<-unique(gsub(".*highDiversity_.*_q(\\d+)_c(\\d)_\\d_summary.txt_\\d","\\1",hnames))
cs<-unique(gsub(".*highDiversity_.*_q(\\d+)_c(\\d)_\\d_summary.txt_\\d","\\1",hnames))
```

```{r highDivQTLs1, fig.cap="Final generation frequencies based on the model with 1 QTLs, with parameters that are expected to result in high diversity based on the mathematical model. In 'monogamy' cases, the males were constrained to only accept eggs from one female into their nests, whereas under 'polygyny' males could mate with multiple females. In both cases females mated once. Parameter settings lablled with 'nm' were runs where females did not mate if they could not find a suitable mate (i.e., if no courters were available)."}

var_patts<-unique(gsub(".*highDiversity_(.*)_\\d_summary.txt_\\d","\\1",
                       hnames[grep(paste0("q",qs[1]),hnames)]))
...
var_patts<-unique(gsub(".*highDiversity_(.*)_\\d_summary.txt_\\d","\\1",
                       hnames[grep(paste0("q",qs[5]),hnames)]))
...
var_patts<-unique(gsub(".*highDiversity_(.*)_\\d_summary.txt_\\d","\\1",
                       hnames[grep(paste0("q",qs[2]),hnames)]))
...
var_patts<-unique(gsub(".*highDiversity_(.*)_\\d_summary.txt_\\d","\\1",
                       hnames[grep(paste0("q",qs[3]),hnames)]))
...
var_patts<-unique(gsub(".*highDiversity_(.*)_\\d_summary.txt_\\d","\\1",
                       hnames[grep(paste0("q",qs[4]),hnames)]))
...
```{r highDivQTLs1super, fig.cap="Final generation frequencies based on the model with 1 QTLs in a supergene, with parameters that are expected to result in high diversity based on the mathematical model. In 'monogamy' cases, the males were constrained to only accept eggs from one female into their nests, whereas under 'polygyny' males could mate with multiple females. In both cases females mated once. Parameter settings lablled with 'nm' were runs where females did not mate if they could not find a suitable mate (i.e., if no courters were available)."}
...
saveRDS(fitness_dat, "fitness_dat.RDS")
```
```{r readFitnessInfo}
fitness_dat<-readRDS("fitness_dat.RDS")
```


```{r fitnessPlotFXN}
fitness_plot<-function(fitness_dat,cols2, pch=19){
  ...
  fitdf<-data.frame(matrix(ncol = ncol(fitness_dat[[1]])))
  colnames(fitdf)<-colnames(fitness_dat[[1]])
  for(i in 1:length(fitness_dat)){
    ...
    if(nrow(fitdat12000)>0){
      ...
      apply(fitdat12000,1,function(dat){
        ...
        lines(which(!is.na(dat[1:4])),
              dat[1:4][!is.na(dat[1:4])],
              lwd=2,
              col=scales::alpha("grey",0.5))
        })
      
      fitdf<-rbind(fitdf,fitdat12000)
    }
    
  }
  fitdf<-fitdf[-1,] # remove initial NA row
  return(fitdf)

}
```

```{r fitnessPlots, fig.cap="Plot of average lifetime reproductive success (and SEM) for each male morph in each replicate in the final generation of the simulations. The lines connect points from within a replicate. Point size reflects the frequency of that morph in the replicate. These graphs show that the high diversity scenarios generally maintain a stable polymorphism of two or more morphs at high frequency due to balancing selection (i.e., equivalent fitness of morphs). The low diversity scenarios occasionally have low frequencies of other morphs, but they tend to be the result of recombination and not balancing selection (i.e., their fitnesses are substantially lower than the high frequency CP morph)."}

par(mfrow=c(2,2), mar=c(2,2,2,2),oma=c(2,2,1,1))

# High diversity, QTLS
hq<-fitness_plot(fitness_dat[grep("qtls//high",names(fitness_dat))], cols2, pch=16)
mtext("QTLs, high diversity",3)

# High diversity, supergene
hs<-fitness_plot(fitness_dat[grep("supergene//high",names(fitness_dat))], cols2, pch=16)
mtext("supergene, high diversity",3)

# Low diversity, QTLS
lq<-fitness_plot(fitness_dat[grep("qtls//low",names(fitness_dat))], cols2, pch=17)
mtext("QTLs, low diversity",3)

# Low diversity, supergene
ls<-fitness_plot(fitness_dat[grep("supergene//low",names(fitness_dat))], cols2, pch=17)
mtext("supergene, low diversity",3)

mtext("Morph",1,outer=TRUE)
mtext("Lifetime Reproductive Success",2,outer=TRUE)
