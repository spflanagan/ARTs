...
What these results suggest is that under polygyny, the density-dependent nature of the model allows males to escape the costs of courtship, because even though they 'use up' all of their sperm, offspring are created in proportion to the surviving offspring, and those males are able to escape the costs and therefore increase their reproductive success beyond what is possible for males in the mathematical model. If we impose stricter costs on the males ($r=0.25$, or $r_{courter}=2$ and $r_{non-courter}=8$), we can see that multiple morphs are maintained under polygyny (with non-random mating, Fig. \@ref(fig:highDiversityStrictPlot)).
...
```{r lowDivGenetics}
lowDiv<-get.morph.freqs(plot.pc.reps(pattern="lowDiversity.*monogamy_nm_q8_c2.*summary.txt",path="../fixedART-results/qtls/",cols,make.plot=FALSE))
```
...
var_patts<-unique(gsub(".*lowDiversity_qtls_monogamy_nm_q8_c2_(.*)_\\d_summary.txt_\\d","\\1",rownames(lowDiv)))
...
```{r highDivGenetics}
highDiv<-get.morph.freqs(plot.pc.reps(pattern="highDiversity.*monogamy_nm_q8_c2.*summary.txt",path="../fixedART-results/qtls/",cols,make.plot=FALSE))
```
...
var_patts<-unique(gsub(".*highDiversity_qtls_monogamy_nm_q8_c2_(.*)_\\d_summary.txt_\\d","\\1",rownames(highDiv)))
...
var_patts<-unique(gsub(".*lowDiversity_qtls_monogamy_nm_prop0.25_q8_c2_(.*)_\\d_summary.txt_\\d","\\1",rownames(lowDiv)))
...
var_patts<-unique(gsub(".*highDiversity_qtls_monogamy_nm_prop0.25_q8_c2_(.*)_\\d_summary.txt_\\d","\\1",rownames(highDiv)))
