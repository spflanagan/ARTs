...
```
```{r librarysetup, echo=FALSE,message=FALSE}
...
```{r readScripts, echo=FALSE,message=FALSE}
...
We want to be able to link empirical data to our model predictions. To do that, we want to be able to look at patterns of genetic variation genome-wide. First we will walk through a single example, and then extrapolate to a broader set of simulation outputs. 
...
So this works well for visualising, but in this particular example the true QTLs aren't identified as outliers. I would like to find a way to summarize whether QTLs are outliers or not across runs. 

If I use a similar approach as I did for my gwsca model (Flanagan & Jones 2015), I want to find peaks and identify if a QTL exists in the peak. In my previous model, I hard-coded that in C++, but will implement it in R for this model. First, I find peaks in distributions using a function and then identify whether my QTLs fall within those peaks.
...
In the specific example from above, we can visualise how this works: 
...
and extract the QTLs that are in the identified peaks. 
...
Now that we are confident in this method, we can write a function to apply it to all the model outputs. 
...
...and then run that function on each of the sets of runs. 


```{r runQTLsHigh, eval=FALSE}
...
```{r runQTLsLow, eval=FALSE}
qtl_outliers<-dplyr::bind_rows(lapply(
  list.files(path="/mnt/BigData/fixedART-results/qtls", 
             pattern = ".*lowDiversity_qtls_monogamy_nm.*.vcf",
             full.names = TRUE),
  get_summary))
write.csv(qtl_outliers,"qtl_outliers.csv",quote=FALSE,row.names = FALSE, append = TRUE)

```


```{r runSupergenesHigh,eval=FALSE}
...
```{r runSupergenesLow,eval=FALSE}

supergene_outliers<-dplyr::bind_rows(lapply(
  list.files(pattern=".*lowDiversity.*monogamy_nm.*.vcf",
             path= "/mnt/BigData/fixedART-results/supergene/", 
             full.names = TRUE),
  get_summary))
write.csv(supergene_outliers,"supergene_outliers.csv",quote=FALSE,row.names = FALSE, append=TRUE)

```

Since there is a lot of data to parse, I have saved the outliers in csv files that I can now read back into R.
...
To see if heterozygosity differs significantly, I can do a t-test with the Welch-Satterthwaite correction using the summary statistics I generated previously and saved int he csv files. 
...
Are parent QTLS more likely to be picked up because CP/CN is the most common polymorphism or is it because selection on parents is stronger than selection on courters? To answer this, we can summarize the final frequencies and compare those to the prop QTL detected (Fig. \@ref(fig:freqsDetection)).
...
I've estimated LD using vcftools (`../scripts/103_calc-LD.sh`), so I simply need to summarise those outputs here.  
...
write.csv(lds,"ld_summary_resampled.csv", row.names = TRUE,quote=FALSE)
```
```{r readLDresampled}
...
LD is highest in low diversity scenarios and with QTLs, which is not what was expected -- I would have predicted supergenes would have the highest LD.


# Tajima's D

I used a script, `105_summarize-TajimaD.R`, to loop over all the Tajima's D output files from vcftools and write some summary statistics to a file. I will then analyse these here. 
...
Some of the results documented here are non-intuitive, such as the patterns of LD and the levels of genetic diversity. To better understand the dynamics, we can explore a single example -- in this case, a set of replicates with 8 QTLs and 2 chromosomes. 
...
court_scan <- GWASpoly(data=data.loco,
                           models=c("additive","1-dom"),
                           traits=c("CourtTrait","ParentTrait"),
                           params=params
                       )
data2 <- set.threshold(court_scan,method="M.eff",level=0.05)
...
```{r supergeneLD, eval=FALSE}
ld_file<-gsub(".vcf",".LD.geno.ld",vcfname)
ld_info<-read.delim(ld_file)
...
write.table(ld_summary,paste0(ld_file,"_summary.txt"),
            row.names = FALSE,
            col.names = TRUE)

```
```{r readSupergeneLD}
ld_summary<-read.delim(gsub(".vcf",".LD.geno.ld_summary.txt",vcfname),
                       sep=' ')
```
...
```{r supergeneFig}
par(mfrow=c(6,1),mar=c(1,4.5,2,2),oma=c(2,3.5,1,1), xpd=TRUE)

chrIndex<-factor(gsub("(\\d)_\\d+","\\1",colnames(data2@geno)))
# GWAS courter
plot(data2@scores$CourtTrait$additive,
     ...
     xaxt='n',
     pch=19,
     col=c("grey73","grey44")[chrIndex],
     bty='l',
     cex.lab=1.5)
mtext("Courter",2,line=5)
qtllocs<-qtls[qtls$trait=="C","loc"]
qtlvals<-data2@scores$CourtTrait$additive[qtllocs]
qtlvals[is.na(qtlvals)]<-0
points(qtllocs, 
       qtlvals,
       lwd=6,
       col=cols["courter"],
...
# GWAS parent
plot(data2@scores$ParentTrait$additive,
     ylab=expression(-log[10](p)),
     xlab="",
     xaxt='n',
     pch=19,
     ...
     bty='l',
     cex.lab=1.5)
mtext("Parent",2,line=5)
qtllocs<-qtls[qtls$trait=="P","loc"]
qtlvals<-data2@scores$ParentTrait$additive[qtllocs]
qtlvals[is.na(qtlvals)]<-0
points(qtllocs, 
       qtlvals,
       lwd=6,
...
plot(mtm_diff$Gprimest,pch=19,
     ...
     xaxt='n',
     bty='l',
     col=c("grey73","grey44")[factor(mtm_diff$CHROM)],
     cex.lab=1.5)

points(qtls$loc, 
       mtm_diff$Gprimest[qtls$loc],cex=2,
       col=qtls$col,lwd=6,
       pch=2)
#abline(h=mean(mtm_diff$Gprimest, na.rm = TRUE),col="grey",lty=2,lwd=2)
#abline(h=median(mtm_diff$Gprimest, na.rm = TRUE),col="grey",lty=3,lwd=2)
...
plot(td$TajimaD,
     ...
     xaxt='n',
     pch=19,
     col=c("grey73","grey44")[factor(td$CHROM)],
     bty='l',
     lwd=2,
     cex.lab=1.5
)
lines(1:nrow(td),
      ...
      col="grey73",
      lwd=2)
lines(which(td$CHROM==1),
       td$TajimaD[td$CHROM==1],
      col="grey44",
      lwd=2)
points(which(td$QTL==TRUE & td$CHROM==0),
       td$TajimaD[td$QTL==TRUE & td$CHROM==0],
       cex=2,
       col=qtls$col[qtls$chrom==0][order(qtls$qtl[qtls$chrom==0])],
       lwd=6,
       pch=2)
points(which(td$QTL==TRUE & td$CHROM==1),
       td$TajimaD[td$QTL==TRUE & td$CHROM==1],
       cex=2,
       col=qtls$col[qtls$chrom==1][order(qtls$qtl[qtls$chrom==1])],
       lwd=6,
       pch=2)
...
plot(mf_diff$Ht,
     ...
     bty='l',
     lwd=2,
     col=c("grey73","grey44")[factor(mf_diff$CHROM)],
     xlab="",
     ylab=expression(italic("H"["T"])),
     xaxt='n',
     cex.lab=1.5)
points(qtls$loc, mf_diff$Ht[qtls$loc],
       cex=2,
       col=qtls$col,
       lwd=6,
       pch=2)
...
# LD
plot(ld_summary$AvgR2,
     type='l',
     bty='l',
     lwd=2,
     col=c("grey73","grey44")[ld_summary$CHR+1],
     xlab="",
     ylab="Mean LD",
     xaxt='n',
     cex.lab=1.5)
lines(which(ld_summary$CHR==1),
       ld_summary$AvgR2[ld_summary$CHR==1],
      col="grey44",
      lwd=2)
points(qtls$loc,
       ld_summary$AvgR2[qtls$loc],
       cex=2,
       col=qtls$col,
       lwd=6,
       pch=2)
...
## Final figure for the main text

```{r combinePopgen, eval=FALSE}
qtl_outliers<-read.csv("qtl_outliers.csv")
qtl_outliers$Architecture<-"qtls"
supergene_outliers<-read.csv("supergene_outliers.csv")
supergene_outliers$Architecture<-"supergene"
...
popgen_data$Diversity<-factor(gsub("^.*/(\\w+)Diversity.*$","\\1",popgen_data$sim),
                              levels=c("low","high"))
write.csv(popgen_data,"popgen_summary.csv", quote = FALSE, row.names = FALSE)
```


```{r getPopgen}
popgen_data<-read.csv("popgen_summary.csv")
popgen_data$Diversity<-factor(popgen_data$Diversity,
                              levels=c("low","high"))
...
td_freqs<-merge(td_summary, final_freqs,by="rep")
write.csv(td_freqs,"td_freqs.csv", quote=FALSE, row.names = FALSE)
```


```{r getTajD}
td_freqs<-read.csv("td_freqs.csv")
td_freqs$diversity<-factor(td_freqs$diversity,
                           levels=c("low","high"))
```

```{r}
div_mod<-lm(popgen_data$gHtMean~popgen_data$Diversity*popgen_data$Architecture)
```
...
```{r genomewideDiv, fig.cap="Genome-wide genetic diversity in the final generation of the simulations was not influcenced by genetic architecture or the phenotypic diversity (i.e., morph frequencies), but did differ between low- and high-diversity parameter settings. Shown are genome-wide observed heterozygosity (A), Tajima’s D (B), and male-male Gst (C).", fig.keep='last', fig.height=3.5, fig.width=10, eval=TRUE}
