...
```{r}
par(mfrow=c(5,4),mar=c(2,2,2,2),oma=c(2,2,2,2))
l<-lapply(pss,function(rep.summ){
  plot(rep.summ$Generation,rep.summ$ParentAEsd,type="l",col=cols["parent"])
  abline(h=rep.summ$ParentThresh[1],lty=2,col="red")
})
```
...
```{r}
par(mfrow=c(5,4),mar=c(2,2,2,2),oma=c(2,2,2,2))
l<-lapply(s,function(rep.summ){
  plot(rep.summ$Generation,rep.summ$ParentAEsd,type="l",col=cols["parent"],
       ylim=c(min(rep.summ$ParentAEsd,rep.summ$CourterAEmean)-0.1,
              max(rep.summ$ParentAEsd,rep.summ$CourterAEmean)+0.1))
